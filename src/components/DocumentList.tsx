...
const DocumentList = ({ isDark }: { isDark: boolean }) => {
    ...
    return (
        <div className={`border-r p-6 space-y-5 h-full overflow-y-auto ${isDark ? 'bg-gray-900 text-white' : 'bg-white text-gray-800'}`}>
            ...
            <div className="space-y-4">
                {filtered.length === 0 ?
                    ...
                    <React.Fragment>
                        ...
                        {/* Search input to filter documents */}
                        <input
                            type="text"
                            value={searchQuery}
                        ...
                        {filtered.map((doc) => (
                            <div
                                key={doc?.id}
                                className={`p-4 rounded-lg border shadow-sm transition-all ${doc?.id === activeId ?
